...
//     </Container>
//     ):(
//      <button> <Link to='/addproduct'> click here</Link></button>
//     )}
//     </>
//   );
// };
...
export default function RemoveProduct() {
    ...
    const [products, setProducts] = useState(() => {
        const savedProducts = localStorage.getItem('saveditem');
    ...
    const handleDeleteProduct = (productId) => {
        const productToDelete = products.find(product => product.id === productId);
        if (!productToDelete) {
            // Notify the user if the product is not found
            alert("Product not found.");
            return;
        }
        ...
        const confirmDelete = window.confirm(`Are you sure you want to delete ${productToDelete.productName}?`);
        if (confirmDelete) {
            // Delete the product and update state and localStorage
            const updatedProducts = products.filter(product => product.id !== productId);
            setProducts(updatedProducts);
            saveProductsToLocal(updatedProducts);
    ...
    const saveProductsToLocal = (updatedProducts) => {
        localStorage.setItem('saveditem', JSON.stringify(updatedProducts));
    };
                                    ...
                                    <Card 
                                      ...
                                      <Card.Body>
                                        <Card.Title 
                                          style={{ fontSize: '20px',color:'white',paddingBottom:'20px', fontFamily: 'fantasy' }}>price </Card.Title>
                                        <Card.Text 
                                          style={{ fontSize: '20px',color:'white', fontFamily: 'fantasy' }}>${calculateTotalPrice().toFixed(2)}</Card.Text>
                                      </Card.Body>
                                    </Card>
                                  </div>
                                        ...
                                        <tbody>
                                            {products.map((product, index) => (
                                                <tr key={product.id}>
                                                    ...
                                                    <td colSpan="3">${calculateProductTotal(product).toFixed(2)}</td>
                                                    <td>
                                                        <button
                                                            className="availability-button"
                                                            onClick={() => alert(`Product ${product.productName} is available in inventory.`)}
                                                        >
                                                            Available
                                                        </button>
                                                    </td>
